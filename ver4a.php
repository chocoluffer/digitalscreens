...
<?php
/**
* Returns an array with each meal period (breakfast, lunch and dinner) based on the file extension given
* Parameter: appropriate file extension
*/
function createArray($file, $optionGiven) {
	...
	$todayDate = date("m/d/y");                          // Gets the current date to split the spreadsheet by date
	$months = array("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec");
	$month = $todayDate[0] . $todayDate[1];
	$thisMonth = $months[$month-1];                     // Gets the month in the proper format that the spreadsheet uses

	$data = explode($thisMonth, $handle);   // Splits the spreadsheet into an array based on the month, which is the
    	                                        //first word on each line
	$size = count($data);                   // Gets size of items on the spreadsheet
	...
	for($i = 1; $i < $size; $i++) {             // Iterates through the text file line by line. Index 0 will always be empty.
    	...
    	$formalName = $dataItem[2]; 
    
    	$mealTimes = explode('/', $dataItem[4]);
    	$times = count($mealTimes);
    	
    	if(strpos($file, "mss") !== false) {
    		$optionSplit = "mss";
    	} else {
...
function handleCase($optionMenu, $caseInfo) {
	$adjustedMenu;
	$optionLen = count($optionMenu);
	$caseLen = count($caseInfo);
	$stopAdd;
	$titleAdded;
	$caseDone = false;
	...
	foreach($optionMenu as $item) {
		...
		foreach($caseInfo as $case) {			
			if(strpos($item[2], $case[0]) !== false) {
				
				// Simplified to it's title...
				if(strpos($case[1], "simplify") !== false) {
					$stopAdd[] = $item[2];
					
					if(strpos($item[2], "fruit") !== false) {
						if($adjustedMenu === null) {
							...
							$adjustedMenu[$item[0]] = $item;
						}
					} else if(strpos($item[2], "salad") !== false) {
						if($adjustedMenu === null) {
							$adjustedMenu = array
								(
								"Salad Bar" => array("Salad Bar","{item[1]}","salad bar", "Animal")
						...
						if($adjustedMenu === null) {
							$adjustedMenu = array
								(
								"{item[2]}" => array("{item[2]}","{item[1]}","{item[2]}", "Animal")
								);
						} else {
							$adjustedMenu[$item[2]] = array("{item[2]}","{item[1]}","{item[2]}", "Animal");
						}
					}
					
				} else if(strpos($case[1], "title") !== false) {
					$dontAdd = false;
					foreach($titleAdded as $addedTitle) {
						if(strpos($addedTitle, $case[0]) !== false) {
							$dontAdd = true;
						} 
					} // end foreach $titledAdded
					
					if($dontAdd === false) {
						$titleAdded[] = $case[0];
						// add title and carry on
						
					}
					
					// Do the stuffs for title
				...
				$caseDone = true;
				break;
			} // end if the case and mealType line up
		} // end foreach caseInfo
		
		// If it's not one of the cases, add it back in as normal
		if($caseDone == false) {
			if($adjustedMenu === null) {
				$adjustedMenu = array
					(
		...
		// Unset caseDone again to ensure nothing funky happens
		$caseDone = false;
	} // end foreach optionMenu
